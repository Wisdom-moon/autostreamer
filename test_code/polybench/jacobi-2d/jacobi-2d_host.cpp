			    ...
			    DATA_TYPE POLYBENCH_2D(B,N,N,n,n))
{
  ...
  (hStreams_app_create_buf((double (*)[1300])A, (1+((_PB_N - 1)-1)+ 1)* sizeof (double [1300])));
  (hStreams_app_create_buf((double (*)[1300])B, (((_PB_N - 1)-1)+ 1)* sizeof (double [1300])));
  int t, i, j;

  for (t = 0; t < _PB_TSTEPS; t++)
    {
(hStreams_app_xfer_memory((double (*)[1300])A, (double (*)[1300])A ,(1+((_PB_N - 1)-1)+ 1)* sizeof (double [1300]), 0, HSTR_SRC_TO_SINK, NULL));
...
for (int idx_subtask = 0; idx_subtask < 4; idx_subtask++)
{
  ...
  (hStreams_EnqueueCompute(
			...
			args,
			NULL,NULL,0));
  (hStreams_app_xfer_memory(&B[start_index][0], &B[start_index][0], (end_index - start_index) * sizeof (double [1300]), idx_subtask % 2, HSTR_SINK_TO_SRC, NULL));
