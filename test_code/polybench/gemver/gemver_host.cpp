		   ...
		   DATA_TYPE POLYBENCH_1D(z,N,n))
{
  ...
  (hStreams_app_create_buf((double *)w, (((n)-1)+ 1)* sizeof (double )));
...
for (int idx_subtask = 0; idx_subtask < 4; idx_subtask++)
{
  ...
  (hStreams_app_xfer_memory(&w[start_index], &w[start_index], (end_index - start_index) * sizeof (double ), idx_subtask % 2, HSTR_SRC_TO_SINK, NULL));
  (hStreams_EnqueueCompute(
			...
			args,
			NULL,NULL,0));
  (hStreams_app_xfer_memory(&w[start_index], &w[start_index], (end_index - start_index) * sizeof (double ), idx_subtask % 2, HSTR_SINK_TO_SRC, NULL));
