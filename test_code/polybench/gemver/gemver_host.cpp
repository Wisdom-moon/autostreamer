		   ...
		   DATA_TYPE POLYBENCH_1D(z,N,n))
{
  ...
  cl_mem w_mem_obj = clCreateBuffer(clGPUContext, CL_MEM_READ_WRITE, (((n)-1)+ 1)* sizeof (double ), NULL, NULL);
...
clSetKernelArg(clKernel, 2, sizeof(cl_mem), (void *) &w_mem_obj);
...
for (int i = 0; i < tasks; i++)
{
  ...
  clEnqueueWriteBuffer(clCommandQue[i], A_mem_obj, CL_FALSE, i*(((n)-1)+ 1)* sizeof (double [2000])/tasks, (((n)-1)+ 1)* sizeof (double [2000])/tasks, &A[i*(((n)-1)-0 + 1)/tasks][0], 0, NULL, NULL);
  clEnqueueWriteBuffer(clCommandQue[i], w_mem_obj, CL_FALSE, i*(((n)-1)+ 1)* sizeof (double )/tasks, (((n)-1)+ 1)* sizeof (double )/tasks, &w[i*(((n)-1)-0 + 1)/tasks], 0, NULL, NULL);
  clEnqueueNDRangeKernel(clCommandQue[i], clKernel, 1, globalOffset, globalThreads, localThreads, 0, NULL, NULL);
  clEnqueueReadBuffer(clCommandQue[i], w_mem_obj, CL_FALSE, i*(((n)-1)+ 1)* sizeof (double )/tasks, (((n)-1)+ 1)* sizeof (double )/tasks, &w[i*(((n)-1)-0 + 1)/tasks], 0, NULL, NULL);
}
for (int i = 0; i < tasks; i++)
