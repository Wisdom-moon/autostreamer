        ...
        int max_rows, int max_cols, int penalty)
{
    ...
    (hStreams_app_create_buf((int *)input_itemsets, (++((16)-1)+1+ 1)* sizeof (int )));
    (hStreams_app_create_buf((int *)referrence, (++((16)-1)+1+ 1)* sizeof (int )));
    for( int blk = 1; blk <= (max_cols-1)/BLOCK_SIZE; blk++ )
    {
(hStreams_app_xfer_memory((int *)input_itemsets, (int *)input_itemsets ,(++((16)-1)+1+ 1)* sizeof (int ), 0, HSTR_SRC_TO_SINK, NULL));
(hStreams_app_xfer_memory((int *)referrence, (int *)referrence ,(++((16)-1)+1+ 1)* sizeof (int ), 0, HSTR_SRC_TO_SINK, NULL));
int sub_blocks = (((())-1)-0 + 1)/ 4;
int remain_index = (((())-1)-0 + 1)% 4;
int start_index = 0;
int end_index = 0;
uint64_t args[8];
args[2] = (uint64_t) blk;
args[3] = (uint64_t) max_cols;
args[4] = (uint64_t) penalty;
args[5] = (uint64_t) max_rows;
args[6] = (uint64_t) referrence;
args[7] = (uint64_t) input_itemsets;
...
hStreams_ThreadSynchronize();
  (hStreams_app_xfer_memory((int *)input_itemsets, (int *)input_itemsets ,(++((16)-1)+1+ 1)* sizeof (int ), 0, HSTR_SINK_TO_SRC, NULL));
    }    
